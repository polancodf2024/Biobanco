...
def generar_identificacion(prefijo):
    ...
    # Insertar un nuevo registro en la tabla y obtener el último ID generado
    cursor.execute("INSERT INTO identificaciones (prefijo) VALUES (?)", (prefijo,))
    conn.commit()  # Confirmar la inserción en la base de datos

    # Obtener el último ID generado (autoincremental)
    nuevo_id = cursor.lastrowid

    # Formatear la identificación con el prefijo y ceros a la izquierda
...
with st.form(key='cuestionario_form'):
    ...
    # Pregunta 30: Identificación de la muestra
    # Lógica para gestionar el prefijo según la procedencia seleccionada
    if responses['Procedencia del paciente'] == 'Donador Control':
        prefijo = 'CB'
    else:
        prefijo = st.selectbox(
                'Si "Procedencia del Paciente = Donador Control", implica que  "Identificación de la muestra = CB"',
            ['PB', 'CB']
    )
    ...
    # Guarda la identificación generada en el diccionario de respuestas
    responses['Identificación de la muestra'] = identificacion
