...
# Función para generar la identificación única
def generar_identificacion(prefijo):
    numero = random.randint(0, 999999)  # Genera un número aleatorio de 6 dígitos
    return f"{prefijo}-{numero:06d}"  # Formato: PB-000001 o CBB-123456
...
with st.form(key='cuestionario_form'):
    ...
    # Pregunta 25: ¿El paciente tiene dislipidemia?
    ...
    # Pregunta 4: Identificación de la muestra
#    responses['Identificación de la muestra'] = st.text_input('Identificación de la muestra')

    # Selección del prefijo por el usuario
    prefijo = st.selectbox('Seleccione el prefijo de la muestra', ['PB', 'CBB'])

    # Generar la identificación con el prefijo seleccionado
    id_muestra = generar_identificacion(prefijo)
    st.write(f'Identificación generada: {id_muestra}')
    responses['Identificación de la muestra'] = id_muestra

    print(id_muestra)  # Ejemplo: PB-123456
